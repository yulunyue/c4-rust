...
use crate::{
    closest::*, collision::*, drone::*, entity::*, fish::*, player::*, scan::*, ugly::*, vector::*,
    xorshift::xorshift,
};
...
pub const STATE_SIZE: usize = INPUT_PER_FISH * 12
    + Game::DRONES_PER_PLAYER as usize * INPUT_PER_DRONE * 2
    + GLOBAL_INPUTS * 2;
...
impl Game {
    ...
    fn init_uglies(&mut self) {
        let ugly_count = if Game::ENABLE_UGLIES {
            1 + self.random.next_in_range(3)
        } else {
            0
        };
        ...
        for _ in 0..ugly_count {
            ...
            let y = (Game::HEIGHT / 2) as u32
                + self.random.next_in_range((Game::HEIGHT / 2) as i32) as u32;
            for k in 0..2 {
        ...
        for col in (0..Game::COLORS_PER_FISH).step_by(2) {
            for type_idx in 0..FishType::variants().len() {
                ...
                while !position_found {
                    x = self
                        .random
                        .next_in_range((Game::WIDTH - Game::FISH_X_SPAWN_LIMIT as i32 * 2) as i32)
                        as u32
                        + Game::FISH_X_SPAWN_LIMIT as u32;
                    //eprintln!("{}", (Game::WIDTH as f64 - Game::FISH_X_SPAWN_LIMIT * 2.0));
                    if type_idx == 0 {
                        y = (1.0 * Game::HEIGHT as f64 / 4.0) as i32
                            + Game::FISH_SPAWN_MIN_SEP as i32;
                        low_y = (1.0 * Game::HEIGHT as f64 / 4.0) as i32;
                        high_y = (2.0 * Game::HEIGHT as f64 / 4.0) as i32;
                    } else if type_idx == 1 {
                        y = (2.0 * Game::HEIGHT as f64 / 4.0) as i32
                            + Game::FISH_SPAWN_MIN_SEP as i32;
                        low_y = (2.0 * Game::HEIGHT as f64 / 4.0) as i32;
                        high_y = (3.0 * Game::HEIGHT as f64 / 4.0) as i32
                    } else {
                        y = (3.0 * Game::HEIGHT as f64 / 4.0) as i32
                            + Game::FISH_SPAWN_MIN_SEP as i32;
                        low_y = (3.0 * Game::HEIGHT as f64 / 4.0) as i32;
                        high_y = (4.0 * Game::HEIGHT as f64 / 4.0) as i32;
                    }
                    y += (self.random.next_in_range(
                        (Game::HEIGHT / 4 - Game::FISH_SPAWN_MIN_SEP as i32 * 2) as i32,
                    ) as u32) as i32;

                    let final_x = x;
                    let final_y = y;
                    let too_close = self.fishes.iter().any(|other| {
                        other.pos.in_range(
                            &Vector::new(final_x as f64, final_y as f64),
                            Game::FISH_SPAWN_MIN_SEP,
                        )
                    });
                    let too_close_to_center =
                        (Game::CENTER.x - x as f64).abs() <= Game::FISH_SPAWN_MIN_SEP;
    ...
    fn move_entities(&mut self) {
        ...
        let fish_to_remove: Vec<i32> = self
            .fishes
            .iter()
        ...
        self.fishes
            .retain(|fish| !fish_to_remove.iter().any(|f_id| *f_id == fish.id));
            ...
            if let Some(target) = ugly.target {
                ...
                if attack_vec.length() > Game::UGLY_ATTACK_SPEED as f64 {
                    attack_vec = attack_vec
                        .normalize()
                        .mult(Game::UGLY_ATTACK_SPEED as f64)
                        .round();
                }
                ...
                if ugly.speed.length() > Game::UGLY_SEARCH_SPEED as f64 {
                    ugly.speed = ugly
                        .speed
                        .normalize()
                        .mult(Game::UGLY_SEARCH_SPEED as f64)
                        .round();
                }

                if !ugly.speed.is_zero() {
                    let closest_uglies =
                        get_closest_to(ugly.pos, uglies_clone.iter().filter(|u| u.id != ugly.id));
                    if !closest_uglies.list.is_empty()
                        && closest_uglies.distance <= Game::FISH_AVOID_RANGE as f64
                    {
                ...
                if (next_pos.x < 0.0 && next_pos.x < ugly.pos.x)
                    || (next_pos.x > Game::WIDTH as f64 - 1.0 && next_pos.x > ugly.pos.x)
                {
        ...
        for fish in &mut self.fishes {
            ...
            if Game::FISH_WILL_FLEE {
                let closest_drones = get_closest_to(
                    ...
                    self.players
                        .iter()
                        .flat_map(|p| p.drones.iter().filter(|d| d.is_engine_on() && !d.dead)),
                );

                if !closest_drones.list.is_empty()
                    && closest_drones.distance <= Game::FISH_HEARING_RANGE as f64
                {
            ...
            if let Some(flee_from_vec) = flee_from {
                ...
                let closest_fishes =
                    get_closest_to(fish.pos, fishes_copy.iter().filter(|f| f.id != fish.id));

                if !closest_fishes.list.is_empty()
                    && closest_fishes.distance <= Game::FISH_AVOID_RANGE as f64
                {
    ...
    fn update_drones(&mut self) {
        for player in &mut self.players {
            for drone in player.drones.iter_mut() {
                let move_speed = (Game::DRONE_MOVE_SPEED
                    - Game::DRONE_MOVE_SPEED
                        * Game::DRONE_MOVE_SPEED_LOSS_PER_SCAN
                        * drone.scans.len() as f64) as f64;
    ...
    fn do_scan(&mut self) {
        for player in self.players.iter_mut() {
            for drone in player.drones.iter_mut() {
                ...
                let scannable_fish: Vec<&Fish> = self
                    .fishes
                    .iter()
                    .filter(|fish| {
                        fish.pos.in_range(
    ...
    fn apply_scans_for_report(&mut self, player_index: usize, drone_index: usize) -> bool {
            ...
            if !self.first_to_scan.contains_key(scan) {
                if self.first_to_scan_temp.contains_key(scan) {
                    ...
                    self.first_to_scan_temp
                        .insert(scan.clone(), player_index as i32);
                }
            }
        ...
        if !player.drones[drone_index].scans.is_empty() {
            player
                .count_fish_saved
                .push(player.drones[drone_index].scans.len() as i32);
        }
        let size_before = player.scans.len();
        player
            .scans
            .extend(player.drones[drone_index].scans.drain());
        ...
        for color in 0..Game::COLORS_PER_FISH {
            if !self.first_to_scan_all_fish_of_color.contains_key(&color) {
                if self.player_scanned_all_fish_of_color(player_index, color) {
                    if self
                        .first_to_scan_all_fish_of_color_temp
                        .contains_key(&color)
                    {
                        // Opponent also completed this report this turn, nobody gets the bonus
                        ...
                        self.first_to_scan_all_fish_of_color_temp
                            .insert(color, player_index as i32);
                    }
                }
            }
        }
    ...
    fn do_report(&mut self) {
        for player_index in 0..self.players.len() {
            ...
            for drone_index in 0..self.players[player_index].drones.len() {
                ...
                if Game::SIMPLE_SCANS
                    || (!self.players[player_index].drones[drone_index]
                        .scans
                        .is_empty()
                        && self.players[player_index].drones[drone_index].pos.y
                            <= Game::DRONE_START_Y as f64)
                {
    ...
    fn has_scanned_all_remaining_fish(&self, player_index: usize) -> bool {
        self.fishes
            .iter()
            .all(|fish| self.player_scanned(player_index, fish))
    }
    ...
    fn has_fish_escaped(&self, scan: &Scan) -> bool {
        !self
            .fishes
            .iter()
            .any(|fish| fish.color == scan.color && fish.fish_type == scan.fish_type)
    }
    ...
    fn is_fish_scanned_by_player_drone(&self, scan: &Scan, player_index: usize) -> bool {
        self.players[player_index]
            .drones
            .iter()
            .any(|drone| drone.scans.contains(scan))
    }
    ...
    fn is_type_combo_still_possible(&self, player_index: usize, fish_type: &FishType) -> bool {
        ...
        for color in 0..Game::COLORS_PER_FISH {
            ...
            if self.has_fish_escaped(&scan)
                && !self.is_fish_scanned_by_player_drone(&scan, player_index)
                && !self.player_scanned_scan(player_index, &scan)
            {
                return false;
            }
        }
        ...
        for color in 0..Game::COLORS_PER_FISH {
            for fish_type in FishType::variants() {
                ...
                if !self.player_scanned_scan(player_index, &scan) {
                    ...
                    if self.is_fish_scanned_by_player_drone(&scan, player_index)
    ...
    fn both_players_have_scanned_all_remaining_fish(&self) -> bool {
        self.players
            .iter()
            .all(|player| self.has_scanned_all_remaining_fish(player.index as usize))
    }

    fn player_scanned_all_fish_of_color(&self, player_index: usize, color: i32) -> bool {
        FishType::variants().iter().all(|&fish_type| {
            self.player_scanned_scan(player_index, &Scan::new_from_type_color(fish_type, color))
        })
    }
    ...
    pub fn compute_player_score(&self, player_index: usize) -> i32 {
        ...
        for scan in &self.players[player_index].scans {
            ...
            if self.first_to_scan.contains_key(scan)
                && *self.first_to_scan.get(scan).unwrap() == player_index as i32
            {
                total += scan.fish_type as i32 + 1;
            }
        }
        ...
        for fish_type in FishType::variants() {
            ...
            if self.first_to_scan_all_fish_of_type.contains_key(fish_type)
                && *self.first_to_scan_all_fish_of_type.get(fish_type).unwrap()
                    == player_index as i32
            {
                total += Game::COLORS_PER_FISH as i32;
            }
        ...
        for color in 0..Game::COLORS_PER_FISH {
            ...
            if self.first_to_scan_all_fish_of_color.contains_key(&color)
                && *self.first_to_scan_all_fish_of_color.get(&color).unwrap() == player_index as i32
            {
                total += FishType::variants().len() as i32;
            }
        }
    ...
    fn player_scanned_all_fish_of_type(&self, player_index: usize, fish_type: FishType) -> bool {
        (0..Game::COLORS_PER_FISH).all(|color| {
            self.player_scanned_scan(player_index, &Scan::new_from_type_color(fish_type, color))
        })
    }
    ...
    pub fn encode(&self, player: usize) -> [f32; STATE_SIZE] {
        ...
        for f in &self.fishes {
            ...
            inputs[id * INPUT_PER_FISH + 0] = ((if player == 0 {
                f.get_x()
            } else {
                9999.0 - f.get_x()
            }) / 10000.0) as f32;
            inputs[id * INPUT_PER_FISH + 1] = (f.get_y() / 10000.0) as f32;
            inputs[id * INPUT_PER_FISH + 2] =
                ((if player == 0 { f.speed.x } else { -f.speed.x }) / 400.0) as f32;
            inputs[id * INPUT_PER_FISH + 3] = (f.speed.y / 400.0) as f32;

            let is_scanned_player = self.is_fish_scanned_by_player_drone(
                &Scan::new_from_type_color(f.fish_type, f.color),
                player as usize,
            );
            let is_scanned_opp = self.is_fish_scanned_by_player_drone(
                &Scan::new_from_type_color(f.fish_type, f.color),
                1 - player as usize,
            );
        ...
        for player_id in 0..2 {
            ...
            for (i, d) in self.players[pid].drones.iter().enumerate() {
                inputs[player_id as usize * INPUT_PER_DRONE + 0 + INPUT_PER_FISH * 12] =
                    ((if player == 0 {
                        d.get_x()
                    } else {
                        9999.0 - d.get_x()
                    }) / 10000.0) as f32;
                inputs[player_id as usize * INPUT_PER_DRONE + 1 + INPUT_PER_FISH * 12] =
                    (d.get_y() / 10000.0) as f32;
                inputs[player_id as usize * INPUT_PER_DRONE + 2 + INPUT_PER_FISH * 12] =
                    (d.battery as f32 / 30.0) as f32;
                inputs[player_id as usize * INPUT_PER_DRONE + 3 + INPUT_PER_FISH * 12] =
                    if d.is_light_on() { 1.0 } else { 0.0 };
            }
        }

        let dif = (INPUT_PER_FISH * 12 + 2 * INPUT_PER_DRONE * (Game::DRONES_PER_PLAYER as usize))
            as usize;
        ...
        for player_id in 0..2 {
            ...
            inputs[dif + player_id as usize * GLOBAL_INPUTS + 0] =
                self.compute_player_score(pid) as f32 / 96.0;
            inputs[dif + player_id as usize * GLOBAL_INPUTS + 1] =
                self.compute_max_player_score(pid) as f32 / 96.0;
            inputs[dif + player_id as usize * GLOBAL_INPUTS + 2] =
                self.game_turn as f32 / Game::MAX_TURNS as f32;
        }
        inputs
    }
    ...
    pub fn step(&mut self, actions: [Action; 2]) {
        for i in 0..2 {
            self.players[i].drones[0].move_command =
                Some(self.players[i].drones[0].pos.add(actions[i].dir));
